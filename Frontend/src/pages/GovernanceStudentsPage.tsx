...
interface GovernanceStudentsPageProps {
  unitType: "SG" | "ORG";
}
...
const GovernanceStudentsPage = ({ unitType }: GovernanceStudentsPageProps) => {
  ...
  const scope = useMemo(() => {
    if (!schoolId || !accessUnit) return null;
    return {
      schoolId,
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((student) => {
      ...
      const matchesSearch = !searchValue || name.includes(searchValue) || studentId.includes(searchValue);
      ...
      <main className="container py-4 ssg-workspace-main">
        <section className="ssg-page-header">
          <div className="ssg-page-header__copy">
            <p className="ssg-page-eyebrow">{unitType} Students</p>
            <h1>{unitType === "SG" ? "Department student directory" : "Program student directory"}</h1>
            <p>
              View imported students across {campusName} inside the current {unitType} governance scope. Notes and tags
              never change the student&apos;s base role.
            </p>
          </div>
        </section>
        ...
        <section className="ssg-stat-grid">
          ...
          <article className="ssg-stat-card">
            ...
            <span className="ssg-stat-card__hint">
              {canManageStudents ? "Governance notes and tags are editable" : "Student records are visible only"}
            </span>
          </article>
        </section>
        ...
        <section className="ssg-surface-card">
          <div className="ssg-filter-bar">
            <div className="ssg-filter-field">
              <label htmlFor={`${unitType.toLowerCase()}StudentSearch`}>Search students</label>
              <input
                id={`${unitType.toLowerCase()}StudentSearch`}
                value={search}
            ...
            <div className="ssg-filter-field">
              <label htmlFor={`${unitType.toLowerCase()}StudentDepartment`}>Department</label>
              <select
                id={`${unitType.toLowerCase()}StudentDepartment`}
            ...
            <div className="ssg-filter-field">
              <label htmlFor={`${unitType.toLowerCase()}StudentYear`}>Year level</label>
              <select
                id={`${unitType.toLowerCase()}StudentYear`}
                value={yearFilter}
          ...
          <section className="ssg-surface-card ssg-student-detail">
            ...
            <div className="ssg-inline-detail mt-4">
              <section className="ssg-detail-section">
                <label htmlFor={`${unitType.toLowerCase()}StudentTags`}>Governance tags</label>
                ...
                <div className="ssg-tag-input">
                  <input
                    id={`${unitType.toLowerCase()}StudentTags`}
                    value={tagInput}
                    onChange={(event) => setTagInput(event.target.value)}
                    onKeyDown={handleTagKeyDown}
                    placeholder={canManageStudents ? "Type a tag and press Enter" : "Read-only without manage_students"}
              ...
              <section className="ssg-detail-section">
                <label htmlFor={`${unitType.toLowerCase()}StudentNotes`}>Governance notes</label>
                <textarea
                  id={`${unitType.toLowerCase()}StudentNotes`}
                  rows={6}
                  ...
                  placeholder={
                    canManageStudents ? "Add governance notes for this student" : "Notes are read-only without manage_students"
                  }
                  disabled={!canManageStudents}
                />
                <div className="ssg-inline-actions">
                  {canManageStudents ? (
                          ...
                          try {
                            ...
                            setError(
                              requestError instanceof Error
                                ...
                                : "Failed to save governance notes"
                            );
                          }
                        })();
                      }}
                    ...
                    <div className="ssg-muted-note">
                      Governance notes and tags need the manage_students permission.
                    </div>
                  )}
...
export default GovernanceStudentsPage;
